...
public class Database {
    ...
    func createTable(command: String) {
        let createTable = #"CREATE (\w+) \(([\w\s]+(?: INDEXED)?(?:, [\w\s]+(?: INDEXED)?)*)\);"#
        if let createMatch = command.range(of: createTable, options: .regularExpression) {
            ...
            if let match = match {
                ...
                var indexedName : Set<String> = []
                
                for column in columns {
                    if column.contains("INDEXED") {
                        let columnName = column.replacingOccurrences(of: " INDEXED", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
                        columnNames.append(columnName)
                        indexedName.insert(columnName)
                    }
                    else {
                ...
                let table = Record(name: tableName, columns: columnNames, indexedColumns: indexedName)
                tables[tableName] = table
                print("Таблиця '\(tableName)' була створена з колонками: \(columnNames)")
                if !indexedName.isEmpty {
                    print("Індексовані стовпці: \(indexedName.joined(separator: ", "))")
                }
            }
            else {
        ...
        if let insertMatch = command.range(of: insertTable, options: .regularExpression) {
            ...
            if let match = match {
                ...
                let rowIndex = table.rows.count - 1
                    ...
                    // Обновляем индексы
                    for column in table.indexedColumns {
                        if let columnIndex = table.columns.firstIndex(of: column) {
                            let key = values[columnIndex]
                            table.index[column, default: [:]][key, default: []].append(rowIndex)
                        }
                    }
                tables[tableName] = table
...
//                }
                if table.indexedColumns.contains(columnName) {
                    if let indexedData = table.index[columnName] {
                                        // Упорядоченный поиск ключей
                                        let matchingKeys = indexedData.keys.prefix(while: { $0 < variableName })
                                        
                                        // Сбор строк по индексу
                                        fRows = matchingKeys.flatMap { indexedData[$0] ?? [] }.map { table.rows[$0] }
                                    }
                }
                else {
                    fRows = table.rows.filter {
                        row in return row[indexColumn].replacingOccurrences(of: "\"", with: "") < variableName
                    }
                }
    ...
    func selectGroupByWhere(command: String) {
        ...
        if let selectMatchGroupBy = command.range(of: selectTableGroupBy, options: .regularExpression) {
            ...
            if let match = match {
                ...
                if table.indexedColumns.contains(columnName2), let indexedData = table.index[columnName2] {
                    let matchingkeys = indexedData.keys.filter{ $0 < variableName2}
                    for key in matchingkeys {
                        if let rowIndices = indexedData[key] {
                            filteredRows.append(contentsOf: rowIndices.map {table.rows[$0]})
                        }
                    }
                }
                    
                    else { filteredRows = table.rows.filter {
